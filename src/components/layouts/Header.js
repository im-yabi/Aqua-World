    ...
    <nav className="navbar row">
        <div className="col-12 col-md-3 d-flex justify-content-center align-items-center">
          <div className="navbar-brand">
            <Link to="/">
              <img width="100px" alt='Aquva World  Logo' src={image} />
            </Link>
            </div>
        </div>
        ...
        <div className="col-12 col-md-3 mt-4 mt-md-0 text-center">
          ...
          <div style={{ display: 'inline-block', marginRight: '5px' }}>
            <Link to="/cart">
              <FaCartShopping style={{ color:'white', fontSize: '24px' }}/>
            </Link>
            <span className="ml-1" id="cart_count">{cartItems.length}</span>
          </div>
          <TbTruckDelivery style={{ color:'white', fontSize: '35px', marginLeft: '13px' }} onClick={() => window.open('https://stcourier.com/track/shipment', '_blank')} />
        </div>
    </nav>
    )
}